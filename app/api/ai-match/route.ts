...
export async function POST(request: Request) {
  ...
  if (!apiKey) return NextResponse.json({ error: "OPENAI_API_KEY가 Vercel 환경변수에 설정되지 않았습니다." }, { status: 503 });
  ...
  try {
    ...
    const companyText = companies.map((c, i) => `${i + 1}. ${c.name} | ${c.category} | 직무: ${c.roles.join(", ")} | 시장: ${c.markets.join(", ")} | 기준: ${c.point}`).join("\n");
...
이력서에 없는 경력은 추정하지 말고, 불확실하면 반드시 "확인 필요"라고 표시하라.
점수는 0~100의 상대적 1차 스크리닝 점수이며 과장된 고득점을 피하라.
최근 화장품/뷰티 경력이 짧더라도 이전 경력에서 이전 가능한 역량이 명확하면 그 점을 설명하라.
결과는 반드시 JSON 객체 하나만 출력하고 마크다운 코드블록은 사용하지 마라.
...
상위 추천은 최대 10개까지 포함하라.`;
    ...
    catch { parsed = JSON.parse(outputText.replace(/^```json\s*/i, "").replace(/```\s*$/i, "").trim()); }
    return NextResponse.json({ ...parsed, model });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "AI 분석 중 오류가 발생했습니다." }, { status: 500 });
  }
}
